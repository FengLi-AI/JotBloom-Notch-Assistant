...
一个适配 **Mac 与 Windows** 的屏幕顶部小工具，基础功能免费、开源，为轻度创作和日常电脑办公设计：随手记录灵感、找回复制过的内容、收藏常用提示词，再与 AI 继续讨论想法。
...
本仓库持续维护完整、可独立构建的免费基础功能，继续使用 MIT 许可。下载本仓库源码不需要访问任何私有仓库，也不需要购买或激活。

桌宠、刘海拖入灵感及相关动效计划作为可选付费拓展，在独立私有仓库开发，其实现和专属素材不包含在本仓库中。**这些拓展尚未发布，目前发布的 1.0.4 客户端仍免费使用。**

未来官网继续提供统一的官方客户端下载入口：基础功能直接使用，可选付费拓展单独购买激活。当前不改变已发布功能、安装包或下载地址。后续先打磨 Mac，Windows 保留现有开源源码与下载，暂缓新增功能开发。

开发边界、基础功能与拓展的衔接方式见 [仓库边界与协作](docs/REPOSITORY_BOUNDARIES.md)。
