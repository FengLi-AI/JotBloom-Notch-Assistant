        ...
        app.Startup+=async(_,_)=>{
            ...
            try{
                ...
                foreach(string section in new[]{"顶部标签","剪贴板","存储与隐私","AI 接口","系统提示词","关于","通用"}){
                    var pane=Descendants<SettingsPane>(window).Single();var button=Descendants<BloomButton>(pane).First(b=>b.LabelText==section);button.RaiseEvent(new RoutedEventArgs(Button.ClickEvent));await Task.Delay(600);window.UpdateLayout();Capture(window,Path.Combine(output,"settings-"+section+".png"));results.Add("settings_"+section+"_rendered");
                }
