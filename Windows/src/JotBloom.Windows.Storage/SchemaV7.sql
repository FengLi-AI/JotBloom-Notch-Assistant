-- Blank V7 snapshot from Mac DatabaseMigrator.swift. See scripts/check-schema.py.
...
, origin_kind TEXT NOT NULL DEFAULT 'manual', source_clipboard_id INTEGER REFERENCES clipboard_items(id) ON DELETE SET NULL, source_application_name TEXT, source_bundle_identifier TEXT, sort_order INTEGER NOT NULL DEFAULT 0, title_source TEXT NOT NULL DEFAULT 'fallback' CHECK(title_source IN ('ai','fallback','user')), content_revision INTEGER NOT NULL DEFAULT 0, title_revision INTEGER NOT NULL DEFAULT 0, category_revision INTEGER NOT NULL DEFAULT 0, lifecycle_token TEXT NOT NULL DEFAULT '');
