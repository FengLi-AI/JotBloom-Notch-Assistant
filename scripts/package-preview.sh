...
package_name='JotBloom-1.0.7-universal.dmg'
...
trap cleanup EXIT
echo 'Building v1.0.7 (1.0.7 build 1109); does not publish automatically.'
xcodebuild -project "$project_dir/JotBloom.xcodeproj" -scheme JotBloom \
  ...
  MARKETING_VERSION=1.0.7 CURRENT_PROJECT_VERSION=1109 build > "$output_dir/build.log" 2>&1
