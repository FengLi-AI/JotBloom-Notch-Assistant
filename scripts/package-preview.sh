...
package_name='JotBloom-1.0.4-universal.dmg'
...
trap cleanup EXIT
echo 'Building v1.0.4 (1.0.4 build 1106); does not publish automatically.'
xcodebuild -project "$project_dir/JotBloom.xcodeproj" -scheme JotBloom \
  ...
  MARKETING_VERSION=1.0.4 CURRENT_PROJECT_VERSION=1106 build > "$output_dir/build.log" 2>&1
