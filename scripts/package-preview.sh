...
package_name='JotBloom-1.0.6-universal.dmg'
...
trap cleanup EXIT
echo 'Building v1.0.6 (1.0.6 build 1108); does not publish automatically.'
xcodebuild -project "$project_dir/JotBloom.xcodeproj" -scheme JotBloom \
  ...
  MARKETING_VERSION=1.0.6 CURRENT_PROJECT_VERSION=1108 build > "$output_dir/build.log" 2>&1
