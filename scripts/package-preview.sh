...
package_name='JotBloom-1.0.5-universal.dmg'
...
trap cleanup EXIT
echo 'Building v1.0.5 (1.0.5 build 1107); does not publish automatically.'
xcodebuild -project "$project_dir/JotBloom.xcodeproj" -scheme JotBloom \
  ...
  MARKETING_VERSION=1.0.5 CURRENT_PROJECT_VERSION=1107 build > "$output_dir/build.log" 2>&1
...
hdiutil create -volname '萌生 JotBloom' -srcfolder "$stage_dir" -format UDZO -ov "$dmg" > "$output_dir/dmg-create.log" 2>&1
